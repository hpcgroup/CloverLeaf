...
#pragma omp target teams map(alloc: xarea[:field.xarea.N()],yarea[:field.yarea.N()],volume[:field.volume.N()],density0[:field.density0.N()],density1[:field.density1.N()],energy0[:field.energy0.N()],energy1[:field.energy1.N()],pressure[:field.pressure.N()],viscosity[:field.viscosity.N()],xvel0[:field.xvel0.N()],yvel0[:field.yvel0.N()]) firstprivate(y_min,y_max,x_min,x_max,flux_x_stride,vels_wk_stride,dt,flux_y_stride,base_stride)
...
#pragma omp target teams map(alloc: xarea[:field.xarea.N()],yarea[:field.yarea.N()],volume[:field.volume.N()],density0[:field.density0.N()],density1[:field.density1.N()],energy0[:field.energy0.N()],energy1[:field.energy1.N()],pressure[:field.pressure.N()],viscosity[:field.viscosity.N()],xvel0[:field.xvel0.N()],xvel1[:field.xvel1.N()],yvel0[:field.yvel0.N()],yvel1[:field.yvel1.N()]) firstprivate(y_min,y_max,x_min,x_max,flux_x_stride,vels_wk_stride,dt,flux_y_stride,base_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 2); j++) {
      for (int i = (x_min + 1); i < (x_max + 2); i++) {
        double left_flux = (xarea[i + j * flux_x_stride] * (xvel0[i + j * vels_wk_stride] + xvel0[(i + 0) + (j + 1) * vels_wk_stride] +
