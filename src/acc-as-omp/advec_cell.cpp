...
#pragma omp target teams map(alloc: volume[:field.volume.N()],vol_flux_x[:field.vol_flux_x.N()],vol_flux_y[:field.vol_flux_y.N()],pre_vol[:field.work_array1.N()],post_vol[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_x_stride,flux_y_stride)
...
#pragma omp target teams map(alloc: volume[:field.volume.N()],vol_flux_x[:field.vol_flux_x.N()],pre_vol[:field.work_array1.N()],post_vol[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_x_stride)
#pragma omp distribute parallel for simd collapse(2)
      for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
        for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
          pre_vol[i + j * vels_wk_stride] =
...
#pragma omp target teams map(alloc: vertexdx[:field.vertexdx.N()],density1[:field.density1.N()],energy1[:field.energy1.N()],mass_flux_x[:field.mass_flux_x.N()],vol_flux_x[:field.vol_flux_x.N()],pre_vol[:field.work_array1.N()],ener_flux[:field.work_array7.N()]) firstprivate(y_min,y_max,x_min,x_max,flux_x_stride,vels_wk_stride,base_stride,one_by_six)
#pragma omp distribute parallel for simd collapse(2)
...
#pragma omp target teams map(alloc: vertexdx[:field.vertexdx.N()],density1[:field.density1.N()],energy1[:field.energy1.N()],mass_flux_x[:field.mass_flux_x.N()],vol_flux_x[:field.vol_flux_x.N()],pre_vol[:field.work_array1.N()],ener_flux[:field.work_array7.N()]) firstprivate(y_min,y_max,x_min,x_max,base_stride,vels_wk_stride,flux_x_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 2); j++) {
...
#pragma omp target teams map(alloc: post_vol[:field.work_array2.N()],volume[:field.volume.N()],vol_flux_y[:field.vol_flux_y.N()],vol_flux_x[:field.vol_flux_x.N()],pre_vol[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_y_stride,flux_x_stride)
#pragma omp distribute parallel for simd collapse(2)
      for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
        for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
          pre_vol[i + j * vels_wk_stride] =
...
#pragma omp target teams map(alloc: post_vol[:field.work_array2.N()],volume[:field.volume.N()],vol_flux_y[:field.vol_flux_y.N()],pre_vol[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_y_stride)
#pragma omp distribute parallel for simd collapse(2)
      for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
        for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
          pre_vol[i + j * vels_wk_stride] =
    ...
    double *pre_vol = field.work_array1.data;
    double *ener_flux = field.work_array7.data;
#pragma omp target teams map(alloc: vertexdy[:field.vertexdy.N()],density1[:field.density1.N()],energy1[:field.energy1.N()],mass_flux_y[:field.mass_flux_y.N()],vol_flux_y[:field.vol_flux_y.N()],pre_vol[:field.work_array1.N()],ener_flux[:field.work_array7.N()]) firstprivate(y_min,y_max,x_min,x_max,flux_y_stride,vels_wk_stride,base_stride,one_by_six)
#pragma omp distribute parallel for simd collapse(2)
...
#pragma omp target teams map(alloc: vertexdy[:field.vertexdy.N()],density1[:field.density1.N()],energy1[:field.energy1.N()],mass_flux_y[:field.mass_flux_y.N()],vol_flux_y[:field.vol_flux_y.N()],pre_vol[:field.work_array1.N()],ener_flux[:field.work_array7.N()]) firstprivate(y_min,y_max,x_min,x_max,base_stride,vels_wk_stride,flux_y_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 2); j++) {
      for (int i = (x_min + 1); i < (x_max + 2); i++) {
        double pre_mass_s = density1[i + j * base_stride] * pre_vol[i + j * vels_wk_stride];
