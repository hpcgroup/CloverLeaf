...
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()],vol_flux_x[:field.vol_flux_x.N()],volume[:field.volume.N()],pre_vol[:field.work_array5.N()],post_vol[:field.work_array6.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_y_stride,flux_x_stride)
...
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()],vol_flux_x[:field.vol_flux_x.N()],volume[:field.volume.N()],pre_vol[:field.work_array5.N()],post_vol[:field.work_array6.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_x_stride,flux_y_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
      for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
        post_vol[i + j * vels_wk_stride] =
...
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()],volume[:field.volume.N()],pre_vol[:field.work_array5.N()],post_vol[:field.work_array6.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_y_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
      for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
...
#pragma omp target teams map(alloc: vol_flux_x[:field.vol_flux_x.N()],volume[:field.volume.N()],pre_vol[:field.work_array5.N()],post_vol[:field.work_array6.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride,flux_x_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
      for (int i = (x_min - 2 + 1); i < (x_max + 2 + 2); i++) {
        post_vol[i + j * vels_wk_stride] = volume[i + j * base_stride];
...
#pragma omp target teams map(alloc: mass_flux_x[:field.mass_flux_x.N()],node_flux[:field.work_array1.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,flux_x_stride)
#pragma omp distribute parallel for simd collapse(2)
...
#pragma omp target teams map(alloc: density1[:field.density1.N()],node_mass_post[:field.work_array2.N()],node_mass_pre[:field.work_array3.N()],post_vol[:field.work_array6.N()],node_flux[:field.work_array1.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride)
#pragma omp distribute parallel for simd collapse(2)
      for (int j = (y_min + 1); j < (y_max + 1 + 2); j++) {
...
#pragma omp target teams map(alloc: vel1[:vel1_buffer.N()],node_flux[:field.work_array1.N()],node_mass_pre[:field.work_array3.N()],mom_flux[:field.work_array4.N()],celldx[:field.celldx.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,vel1_sizex)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 1 + 2); j++) {
...
#pragma omp target teams map(alloc: vel1[:vel1_buffer.N()],node_mass_pre[:field.work_array3.N()],mom_flux[:field.work_array4.N()],celldx[:field.celldx.N()],node_mass_post[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vel1_sizex,vels_wk_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 1 + 2); j++) {
...
#pragma omp target teams map(alloc: node_flux[:field.work_array1.N()],mass_flux_y[:field.mass_flux_y.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,flux_y_stride)
#pragma omp distribute parallel for simd collapse(2)
      for (int j = (y_min - 2 + 1); j < (y_max + 2 + 2); j++) {
        for (int i = (x_min + 1); i < (x_max + 1 + 2); i++) {
          node_flux[i + j * vels_wk_stride] =
...
#pragma omp target teams map(alloc: node_flux[:field.work_array1.N()],density1[:field.density1.N()],node_mass_post[:field.work_array2.N()],node_mass_pre[:field.work_array3.N()],post_vol[:field.work_array6.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,base_stride)
#pragma omp distribute parallel for simd collapse(2)
...
#pragma omp target teams map(alloc: vel1[:vel1_buffer.N()],node_mass_pre[:field.work_array3.N()],mom_flux[:field.work_array4.N()],celldy[:field.celldy.N()],node_flux[:field.work_array1.N()]) firstprivate(y_min,y_max,x_min,x_max,vels_wk_stride,vel1_sizex)
#pragma omp distribute parallel for simd collapse(2)
...
#pragma omp target teams map(alloc: vel1[:vel1_buffer.N()],node_mass_pre[:field.work_array3.N()],mom_flux[:field.work_array4.N()],node_mass_post[:field.work_array2.N()]) firstprivate(y_min,y_max,x_min,x_max,vel1_sizex,vels_wk_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = (y_min + 1); j < (y_max + 1 + 2); j++) {
      for (int i = (x_min + 1); i < (x_max + 1 + 2); i++) {
        vel1[i + j * vel1_sizex] = (vel1[i + j * vel1_sizex] * node_mass_pre[i + j * vels_wk_stride] +
