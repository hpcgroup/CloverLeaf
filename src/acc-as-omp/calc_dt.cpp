...
#pragma omp target teams map(alloc: xarea[:field.xarea.N()],yarea[:field.yarea.N()],celldx[:field.celldx.N()],celldy[:field.celldy.N()],volume[:field.volume.N()],density0[:field.density0.N()],viscosity[:field.viscosity.N()],soundspeed[:field.soundspeed.N()],xvel0[:field.xvel0.N()],yvel0[:field.yvel0.N()]) map(tofrom: dt_min_val0) firstprivate(y_min,y_max,x_min,x_max,base_stride,dtc_safe,vels_wk_stride,flux_x_stride,dtu_safe,flux_y_stride,dtv_safe,dtdiv_safe) reduction(min: dt_min_val0)
#pragma omp distribute parallel for simd collapse(2) reduction(min: dt_min_val0)
