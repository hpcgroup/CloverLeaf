...
#pragma omp target teams map(alloc: energy0[:field.energy0.N()],density0[:field.density0.N()],xvel0[:field.xvel0.N()],yvel0[:field.yvel0.N()]) firstprivate(yrange,xrange,base_stride,state_energy_0,state_density_0,vels_wk_stride,state_xvel_0,state_yvel_0)
...
#pragma omp target enter data map(to: state_density[:state_density_buffer.N()]) map(to: state_energy[:state_energy_buffer.N()]) map(to: state_xvel[:state_xvel_buffer.N()]) map(to: state_yvel[:state_yvel_buffer.N()]) map(to: state_xmin[:state_xmin_buffer.N()]) map(to: state_xmax[:state_xmax_buffer.N()]) map(to: state_ymin[:state_ymin_buffer.N()]) map(to: state_ymax[:state_ymax_buffer.N()]) map(to: state_radius[:state_radius_buffer.N()]) map(to: state_geometry[:state_geometry_buffer.N()])\

  globals.profiler.host_to_device += timer() - start_time;

#pragma omp target teams map(alloc: state_density[:state_density_buffer.N()]) map(alloc: state_energy[:state_energy_buffer.N()]) map(alloc: state_xvel[:state_xvel_buffer.N()]) map(alloc: state_yvel[:state_yvel_buffer.N()]) map(alloc: state_xmin[:state_xmin_buffer.N()]) map(alloc: state_xmax[:state_xmax_buffer.N()]) map(alloc: state_ymin[:state_ymin_buffer.N()]) map(alloc: state_ymax[:state_ymax_buffer.N()]) map(alloc: state_radius[:state_radius_buffer.N()]) map(alloc: state_geometry[:state_geometry_buffer.N()]) map(alloc: energy0[:field.energy0.N()],density0[:field.density0.N()],xvel0[:field.xvel0.N()],yvel0[:field.yvel0.N()],cellx[:field.cellx.N()],celly[:field.celly.N()],vertexx[:field.vertexx.N()],vertexy[:field.vertexy.N()]) firstprivate(yrange,xrange,state,base_stride,vels_wk_stride)
#pragma omp distribute parallel for simd collapse(2)
    for (int j = 0; j < (yrange); j++) {
      for (int i = 0; i < (xrange); i++) {
