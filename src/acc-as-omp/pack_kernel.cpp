...
#pragma omp target teams map(alloc: left_snd[:left_snd_buffer.N()],field[:field_buffer.N()]) firstprivate(y_min,depth,y_max,y_inc,buffer_offset,x_min,x_inc,field_sizex)
...
#pragma omp target teams map(alloc: left_rcv[:left_rcv_buffer.N()],field[:field_buffer.N()]) firstprivate(y_min,depth,y_max,y_inc,buffer_offset,x_min,field_sizex)
#pragma omp distribute parallel for simd
  for (int k = (y_min - depth + 1); k < (y_max + y_inc + depth + 2); k++) {
    for (int j = 0; j < depth; ++j) {
      int index = buffer_offset + j + k * depth;
...
void clover_pack_message_right(global_variables &globals, int x_min, int x_max, int y_min, int y_max,
  ...
  double *field = field_buffer.data;
  const size_t field_sizex = field_buffer.nX();
#pragma omp target teams map(alloc: right_snd[:right_snd_buffer.N()],field[:field_buffer.N()]) firstprivate(y_min,depth,y_max,y_inc,buffer_offset,x_max,field_sizex)
#pragma omp distribute parallel for simd
  for (int k = (y_min - depth + 1); k < (y_max + y_inc + depth + 2); k++) {
    for (int j = 0; j < depth; ++j) {
      int index = buffer_offset + j + k * depth;
...
void clover_unpack_message_right(global_variables &globals, int x_min, int x_max, int y_min, int y_max,
  ...
  double *field = field_buffer.data;
  const size_t field_sizex = field_buffer.nX();
#pragma omp target teams map(alloc: right_rcv[:right_rcv_buffer.N()],field[:field_buffer.N()]) firstprivate(y_min,depth,y_max,y_inc,buffer_offset,x_max,x_inc,field_sizex)
#pragma omp distribute parallel for simd
  for (int k = (y_min - depth + 1); k < (y_max + y_inc + depth + 2); k++) {
    for (int j = 0; j < depth; ++j) {
      int index = buffer_offset + j + k * depth;
...
void clover_pack_message_top(global_variables &globals, int x_min, int x_max, int y_min, int y_max, clover::Buffer2D<double> &field_buffer,
  ...
  for (int k = 0; k < depth; ++k) {
    ...
    double *field = field_buffer.data;
    const size_t field_sizex = field_buffer.nX();
#pragma omp target teams map(alloc: top_snd[:top_snd_buffer.N()],field[:field_buffer.N()]) firstprivate(x_min,depth,x_max,x_inc,buffer_offset,k,y_max,field_sizex)
#pragma omp distribute parallel for simd
...
#pragma omp target teams map(alloc: top_rcv[:top_rcv_buffer.N()],field[:field_buffer.N()]) firstprivate(x_min,depth,x_max,x_inc,buffer_offset,k,y_max,y_inc,field_sizex)
#pragma omp distribute parallel for simd
    for (int j = (x_min - depth + 1); j < (x_max + x_inc + depth + 2); j++) {
      int index = buffer_offset + k + j * depth;
...
void clover_pack_message_bottom(global_variables &globals, int x_min, int x_max, int y_min, int y_max,
  ...
  for (int k = 0; k < depth; ++k) {
    ...
    double *field = field_buffer.data;
    const size_t field_sizex = field_buffer.nX();
#pragma omp target teams map(alloc: bottom_snd[:bottom_snd_buffer.N()],field[:field_buffer.N()]) firstprivate(x_min,depth,x_max,x_inc,buffer_offset,k,y_min,y_inc,field_sizex)
#pragma omp distribute parallel for simd
    for (int j = (x_min - depth + 1); j < (x_max + x_inc + depth + 2); j++) {
      int index = buffer_offset + k + j * depth;
...
#pragma omp target teams map(alloc: bottom_rcv[:bottom_rcv_buffer.N()],field[:field_buffer.N()]) firstprivate(x_min,depth,x_max,x_inc,buffer_offset,k,y_min,field_sizex)
#pragma omp distribute parallel for simd
    for (int j = (x_min - depth + 1); j < (x_max + x_inc + depth + 2); j++) {
      int index = buffer_offset + k + j * depth;
