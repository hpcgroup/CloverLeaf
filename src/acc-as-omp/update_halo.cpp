...
#pragma omp target teams map(alloc: density0[:field.density0.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *density0 = field.density0.data;
#pragma omp target teams map(alloc: density0[:field.density0.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *density0 = field.density0.data;
#pragma omp target teams map(alloc: density0[:field.density0.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *density0 = field.density0.data;
#pragma omp target teams map(alloc: density0[:field.density0.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density1[:field.density1.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *density1 = field.density1.data;
#pragma omp target teams map(alloc: density1[:field.density1.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *density1 = field.density1.data;
#pragma omp target teams map(alloc: density1[:field.density1.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *density1 = field.density1.data;
#pragma omp target teams map(alloc: density1[:field.density1.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy0[:field.energy0.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *energy0 = field.energy0.data;
#pragma omp target teams map(alloc: energy0[:field.energy0.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *energy0 = field.energy0.data;
#pragma omp target teams map(alloc: energy0[:field.energy0.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *energy0 = field.energy0.data;
#pragma omp target teams map(alloc: energy0[:field.energy0.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy1[:field.energy1.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *energy1 = field.energy1.data;
#pragma omp target teams map(alloc: energy1[:field.energy1.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *energy1 = field.energy1.data;
#pragma omp target teams map(alloc: energy1[:field.energy1.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *energy1 = field.energy1.data;
#pragma omp target teams map(alloc: energy1[:field.energy1.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: pressure[:field.pressure.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *pressure = field.pressure.data;
#pragma omp target teams map(alloc: pressure[:field.pressure.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *pressure = field.pressure.data;
#pragma omp target teams map(alloc: pressure[:field.pressure.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *pressure = field.pressure.data;
#pragma omp target teams map(alloc: pressure[:field.pressure.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: viscosity[:field.viscosity.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *viscosity = field.viscosity.data;
#pragma omp target teams map(alloc: viscosity[:field.viscosity.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *viscosity = field.viscosity.data;
#pragma omp target teams map(alloc: viscosity[:field.viscosity.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *viscosity = field.viscosity.data;
#pragma omp target teams map(alloc: viscosity[:field.viscosity.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: soundspeed[:field.soundspeed.N()]) firstprivate(x_min,depth,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *soundspeed = field.soundspeed.data;
#pragma omp target teams map(alloc: soundspeed[:field.soundspeed.N()]) firstprivate(x_min,depth,x_max,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *soundspeed = field.soundspeed.data;
#pragma omp target teams map(alloc: soundspeed[:field.soundspeed.N()]) firstprivate(y_min,depth,y_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *soundspeed = field.soundspeed.data;
#pragma omp target teams map(alloc: soundspeed[:field.soundspeed.N()]) firstprivate(y_min,depth,y_max,x_max,base_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel0[:field.xvel0.N()]) firstprivate(x_min,depth,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *xvel0 = field.xvel0.data;
#pragma omp target teams map(alloc: xvel0[:field.xvel0.N()]) firstprivate(x_min,depth,x_max,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *xvel0 = field.xvel0.data;
#pragma omp target teams map(alloc: xvel0[:field.xvel0.N()]) firstprivate(y_min,depth,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *xvel0 = field.xvel0.data;
#pragma omp target teams map(alloc: xvel0[:field.xvel0.N()]) firstprivate(y_min,depth,y_max,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel1[:field.xvel1.N()]) firstprivate(x_min,depth,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *xvel1 = field.xvel1.data;
#pragma omp target teams map(alloc: xvel1[:field.xvel1.N()]) firstprivate(x_min,depth,x_max,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *xvel1 = field.xvel1.data;
#pragma omp target teams map(alloc: xvel1[:field.xvel1.N()]) firstprivate(y_min,depth,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *xvel1 = field.xvel1.data;
#pragma omp target teams map(alloc: xvel1[:field.xvel1.N()]) firstprivate(y_min,depth,y_max,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel0[:field.yvel0.N()]) firstprivate(x_min,depth,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *yvel0 = field.yvel0.data;
#pragma omp target teams map(alloc: yvel0[:field.yvel0.N()]) firstprivate(x_min,depth,x_max,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *yvel0 = field.yvel0.data;
#pragma omp target teams map(alloc: yvel0[:field.yvel0.N()]) firstprivate(y_min,depth,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *yvel0 = field.yvel0.data;
#pragma omp target teams map(alloc: yvel0[:field.yvel0.N()]) firstprivate(y_min,depth,y_max,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel1[:field.yvel1.N()]) firstprivate(x_min,depth,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *yvel1 = field.yvel1.data;
#pragma omp target teams map(alloc: yvel1[:field.yvel1.N()]) firstprivate(x_min,depth,x_max,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *yvel1 = field.yvel1.data;
#pragma omp target teams map(alloc: yvel1[:field.yvel1.N()]) firstprivate(y_min,depth,y_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *yvel1 = field.yvel1.data;
#pragma omp target teams map(alloc: yvel1[:field.yvel1.N()]) firstprivate(y_min,depth,y_max,x_max,vels_wk_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_x[:field.vol_flux_x.N()]) firstprivate(x_min,depth,x_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *vol_flux_x = field.vol_flux_x.data;
#pragma omp target teams map(alloc: vol_flux_x[:field.vol_flux_x.N()]) firstprivate(x_min,depth,x_max,y_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *vol_flux_x = field.vol_flux_x.data;
#pragma omp target teams map(alloc: vol_flux_x[:field.vol_flux_x.N()]) firstprivate(y_min,depth,y_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *vol_flux_x = field.vol_flux_x.data;
#pragma omp target teams map(alloc: vol_flux_x[:field.vol_flux_x.N()]) firstprivate(y_min,depth,y_max,x_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_x[:field.mass_flux_x.N()]) firstprivate(x_min,depth,x_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *mass_flux_x = field.mass_flux_x.data;
#pragma omp target teams map(alloc: mass_flux_x[:field.vol_flux_x.N()]) firstprivate(x_min,depth,x_max,y_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + 1 + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *mass_flux_x = field.mass_flux_x.data;
#pragma omp target teams map(alloc: mass_flux_x[:field.vol_flux_x.N()]) firstprivate(y_min,depth,y_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *mass_flux_x = field.mass_flux_x.data;
#pragma omp target teams map(alloc: mass_flux_x[:field.vol_flux_x.N()]) firstprivate(y_min,depth,y_max,x_max,flux_x_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()]) firstprivate(x_min,depth,x_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *vol_flux_y = field.vol_flux_y.data;
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()]) firstprivate(x_min,depth,x_max,y_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *vol_flux_y = field.vol_flux_y.data;
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()]) firstprivate(y_min,depth,y_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *vol_flux_y = field.vol_flux_y.data;
#pragma omp target teams map(alloc: vol_flux_y[:field.vol_flux_y.N()]) firstprivate(y_min,depth,y_max,x_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_y[:field.mass_flux_y.N()]) firstprivate(x_min,depth,x_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_top] == external_face) && (tile_neighbours[tile_top] == external_tile)) {
      ...
      double *mass_flux_y = field.mass_flux_y.data;
#pragma omp target teams map(alloc: mass_flux_y[:field.mass_flux_y.N()]) firstprivate(x_min,depth,x_max,y_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int j = (x_min - depth + 1); j < (x_max + depth + 2); j++) {
        for (int k = 0; k < depth; ++k) {
    ...
    if ((chunk_neighbours[chunk_left] == external_face) && (tile_neighbours[tile_left] == external_tile)) {
      ...
      double *mass_flux_y = field.mass_flux_y.data;
#pragma omp target teams map(alloc: mass_flux_y[:field.mass_flux_y.N()]) firstprivate(y_min,depth,y_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
    ...
    if ((chunk_neighbours[chunk_right] == external_face) && (tile_neighbours[tile_right] == external_tile)) {
      ...
      double *mass_flux_y = field.mass_flux_y.data;
#pragma omp target teams map(alloc: mass_flux_y[:field.mass_flux_y.N()]) firstprivate(y_min,depth,y_max,x_max,flux_y_stride)
#pragma omp distribute parallel for simd
      for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
        for (int j = 0; j < depth; ++j) {
