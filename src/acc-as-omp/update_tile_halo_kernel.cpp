...
#pragma omp target teams map(alloc: density0[:density0_buffer.N()],left_density0[:left_density0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density0_sizex,left_xmax,left_density0_sizex)
...
#pragma omp target teams map(alloc: density1[:density1_buffer.N()],left_density1[:left_density1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density1_sizex,left_xmax,left_density1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy0[:energy0_buffer.N()],left_energy0[:left_energy0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy0_sizex,left_xmax,left_energy0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy1[:energy1_buffer.N()],left_energy1[:left_energy1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy1_sizex,left_xmax,left_energy1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: pressure[:pressure_buffer.N()],left_pressure[:left_pressure_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,pressure_sizex,left_xmax,left_pressure_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: viscosity[:viscosity_buffer.N()],left_viscosity[:left_viscosity_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,viscosity_sizex,left_xmax,left_viscosity_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: soundspeed[:soundspeed_buffer.N()],left_soundspeed[:left_soundspeed_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,soundspeed_sizex,left_xmax,left_soundspeed_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel0[:xvel0_buffer.N()],left_xvel0[:left_xvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel0_sizex,left_xmax,left_xvel0_sizex)
#pragma omp distribute parallel for simd
...
#pragma omp target teams map(alloc: xvel1[:xvel1_buffer.N()],left_xvel1[:left_xvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel1_sizex,left_xmax,left_xvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel0[:yvel0_buffer.N()],left_yvel0[:left_yvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel0_sizex,left_xmax,left_yvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel1[:yvel1_buffer.N()],left_yvel1[:left_yvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel1_sizex,left_xmax,left_yvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_x[:vol_flux_x_buffer.N()],left_vol_flux_x[:left_vol_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_x_sizex,left_xmax,left_vol_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_x[:mass_flux_x_buffer.N()],left_mass_flux_x[:left_mass_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_x_sizex,left_xmax,left_mass_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_y[:vol_flux_y_buffer.N()],left_vol_flux_y[:left_vol_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_y_sizex,left_xmax,left_vol_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_y[:mass_flux_y_buffer.N()],left_mass_flux_y[:left_mass_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_y_sizex,left_xmax,left_mass_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density0[:density0_buffer.N()],right_density0[:right_density0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density0_sizex,right_xmax,right_density0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density1[:density1_buffer.N()],right_density1[:right_density1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density1_sizex,right_xmax,right_density1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy0[:energy0_buffer.N()],right_energy0[:right_energy0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy0_sizex,right_xmax,right_energy0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy1[:energy1_buffer.N()],right_energy1[:right_energy1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy1_sizex,right_xmax,right_energy1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: pressure[:pressure_buffer.N()],right_pressure[:right_pressure_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,pressure_sizex,right_xmax,right_pressure_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: viscosity[:viscosity_buffer.N()],right_viscosity[:right_viscosity_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,viscosity_sizex,right_xmax,right_viscosity_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: soundspeed[:soundspeed_buffer.N()],right_soundspeed[:right_soundspeed_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,soundspeed_sizex,right_xmax,right_soundspeed_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel0[:xvel0_buffer.N()],right_xvel0[:right_xvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel0_sizex,right_xmax,right_xvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel1[:xvel1_buffer.N()],right_xvel1[:right_xvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel1_sizex,right_xmax,right_xvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel0[:yvel0_buffer.N()],right_yvel0[:right_yvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel0_sizex,right_xmax,right_yvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel1[:yvel1_buffer.N()],right_yvel1[:right_yvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel1_sizex,right_xmax,right_yvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_x[:vol_flux_x_buffer.N()],right_vol_flux_x[:right_vol_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_x_sizex,right_xmax,right_vol_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_x[:mass_flux_x_buffer.N()],right_mass_flux_x[:right_mass_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_x_sizex,right_xmax,right_mass_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_y[:vol_flux_y_buffer.N()],right_vol_flux_y[:right_vol_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_y_sizex,right_xmax,right_vol_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_y[:mass_flux_y_buffer.N()],right_mass_flux_y[:right_mass_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_y_sizex,right_xmax,right_mass_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density0[:density0_buffer.N()],top_density0[:top_density0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density0_sizex,top_xmax,top_density0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density1[:density1_buffer.N()],top_density1[:top_density1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density1_sizex,top_xmax,top_density1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy0[:energy0_buffer.N()],top_energy0[:top_energy0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy0_sizex,top_xmax,top_energy0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy1[:energy1_buffer.N()],top_energy1[:top_energy1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy1_sizex,top_xmax,top_energy1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: pressure[:pressure_buffer.N()],top_pressure[:top_pressure_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,pressure_sizex,top_xmax,top_pressure_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: viscosity[:viscosity_buffer.N()],top_viscosity[:top_viscosity_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,viscosity_sizex,top_xmax,top_viscosity_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: soundspeed[:soundspeed_buffer.N()],top_soundspeed[:top_soundspeed_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,soundspeed_sizex,top_xmax,top_soundspeed_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel0[:xvel0_buffer.N()],top_xvel0[:top_xvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel0_sizex,top_xmax,top_xvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel1[:xvel1_buffer.N()],top_xvel1[:top_xvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel1_sizex,top_xmax,top_xvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel0[:yvel0_buffer.N()],top_yvel0[:top_yvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel0_sizex,top_xmax,top_yvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel1[:yvel1_buffer.N()],top_yvel1[:top_yvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel1_sizex,top_xmax,top_yvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_x[:vol_flux_x_buffer.N()],top_vol_flux_x[:top_vol_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_x_sizex,top_xmax,top_vol_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_x[:mass_flux_x_buffer.N()],top_mass_flux_x[:top_mass_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_x_sizex,top_xmax,top_mass_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_y[:vol_flux_y_buffer.N()],top_vol_flux_y[:top_vol_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_y_sizex,top_xmax,top_vol_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_y[:mass_flux_y_buffer.N()],top_mass_flux_y[:top_mass_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_y_sizex,top_xmax,top_mass_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density0[:density0_buffer.N()],bottom_density0[:bottom_density0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density0_sizex,bottom_xmax,bottom_density0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: density1[:density1_buffer.N()],bottom_density1[:bottom_density1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,density1_sizex,bottom_xmax,bottom_density1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy0[:energy0_buffer.N()],bottom_energy0[:bottom_energy0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy0_sizex,bottom_xmax,bottom_energy0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: energy1[:energy1_buffer.N()],bottom_energy1[:bottom_energy1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,energy1_sizex,bottom_xmax,bottom_energy1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: pressure[:pressure_buffer.N()],bottom_pressure[:bottom_pressure_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,pressure_sizex,bottom_xmax,bottom_pressure_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: viscosity[:viscosity_buffer.N()],bottom_viscosity[:bottom_viscosity_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,viscosity_sizex,bottom_xmax,bottom_viscosity_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: soundspeed[:soundspeed_buffer.N()],bottom_soundspeed[:bottom_soundspeed_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,soundspeed_sizex,bottom_xmax,bottom_soundspeed_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel0[:xvel0_buffer.N()],bottom_xvel0[:bottom_xvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel0_sizex,bottom_xmax,bottom_xvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: xvel1[:xvel1_buffer.N()],bottom_xvel1[:bottom_xvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,xvel1_sizex,bottom_xmax,bottom_xvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel0[:yvel0_buffer.N()],bottom_yvel0[:bottom_yvel0_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel0_sizex,bottom_xmax,bottom_yvel0_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: yvel1[:yvel1_buffer.N()],bottom_yvel1[:bottom_yvel1_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,yvel1_sizex,bottom_xmax,bottom_yvel1_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_x[:vol_flux_x_buffer.N()],bottom_vol_flux_x[:bottom_vol_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_x_sizex,bottom_xmax,bottom_vol_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_x[:mass_flux_x_buffer.N()],bottom_mass_flux_x[:bottom_mass_flux_x_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_x_sizex,bottom_xmax,bottom_mass_flux_x_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: vol_flux_y[:vol_flux_y_buffer.N()],bottom_vol_flux_y[:bottom_vol_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,vol_flux_y_sizex,bottom_xmax,bottom_vol_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
...
#pragma omp target teams map(alloc: mass_flux_y[:mass_flux_y_buffer.N()],bottom_mass_flux_y[:bottom_mass_flux_y_buffer.N()]) firstprivate(y_min,depth,y_max,x_min,mass_flux_y_sizex,bottom_xmax,bottom_mass_flux_y_sizex)
#pragma omp distribute parallel for simd
    for (int k = (y_min - depth + 1); k < (y_max + 1 + depth + 2); k++) {
      for (int j = 0; j < depth; ++j) {
