...
static const char* get_type_name(){
    ...
    return typeid(T).name(); //other types
}
...
template <typename T> static inline T *alloc(size_t count) {
  void *p{};
#ifdef CLOVER_MANAGED_ALLOC
  auto result = hipMallocManaged(&p, count * sizeof(T));
#else
  auto result = hipMalloc(&p, count * sizeof(T));
#endif
  if (result != hipSuccess) {
    std::cerr << "Failed to allocate memory of " << count << " bytes: " << hipGetErrorString(result) << std::endl;
    std::abort();
  }
...
template <typename T> struct Buffer1D {
  ...
  Buffer1D(context &, size_t size, T *host_init) : size(size), data(alloc<T>(size)) {
    ...
    printf("[CLOVER_DEBUG] Transfer (H2D): 1D Buffer | Type: %s | Elements: %zu | Total: %zu bytes\n", 
            get_type_name<T>(), size, sz);
  ...
  std::vector<T> mirrored() const {
    ...
    printf("[CLOVER_DEBUG] Transfer (D2H): 1D Buffer Mirror | Type: %s | Elements: %zu | Total: %zu bytes\n", 
            get_type_name<T>(), size, sz);
...
template <typename T> struct Buffer2D {
  ...
  std::vector<T> mirrored() const {
    ...
    size_t sz=buffer.size()*sizeof(T);
    printf("[CLOVER_DEBUG] Transfer (D2H): 2D Buffer Mirror | Type: %s | Dimensions: %zu x %zu | Total: %zu bytes\n", 
            get_type_name<T>(), sizeX, sizeY, sz);
...
static void par_reduce(const F functor, const char *file = CLOVER_BUILTIN_FILE, int loc = CLOVER_BUILTIN_LINE) {
  ...
  if (auto result = hipDeviceSynchronize(); result != hipSuccess) {
    std::cerr << "Reduce kernel at " << file << ":" << loc << " failed: " << hipGetErrorString(result) << std::endl;
  }
#endif
}
...
#ifdef CLOVER_SYNC_ALL_KERNELS
  if (auto result = hipDeviceSynchronize(); result != hipSuccess) {
    std::cerr << "1D kernel at " << file << ":" << loc << " failed: " << hipGetErrorString(result) << std::endl;
  }
#endif
}
...
#ifdef CLOVER_SYNC_ALL_KERNELS
  if (auto result = hipDeviceSynchronize(); result != hipSuccess) {
    std::cerr << "2D kernel at " << file << ":" << loc << " failed: " << hipGetErrorString(result) << std::endl;
  }
#endif
}
